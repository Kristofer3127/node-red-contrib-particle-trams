...
class ParticleBaseNode {
  ...
  makeAPIRequest(variables) {
    const parameters = this.parametersFromVariables(
      variables,
    ...
    const hasMandatoryArguments = this.hasMandatoryArguments(
      this.mandatoryArguments,
      parameters
    );
    ...
    this.api[this.functionName](parameters).then(
      (result) => {
        this.hideStatus();
  ...
  onInput(msg) {
    ...
    let variables = this.mergePropertiesFromMsg(
      this.variables,
